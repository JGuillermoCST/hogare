...
class AdViewModel(ctx: Context): ViewModel() {

    private var requestQueue: RequestQueue = Volley.newRequestQueue(ctx)
    var list = mutableStateListOf<Ad>()
    ...
    private fun getAds() {
        viewModelScope.launch {
            val jsonRequest = JsonArrayRequest(Request.Method.GET, "https://hogare.josecst.com/api/avisos/1", null, { result ->
    ...
    private fun saveList(list: SnapshotStateList<Ad>, data: JSONObject) {
        list.add(
            Ad(
